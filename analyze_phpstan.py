...
try:
    with open('phpstan_results.json', 'r') as f:
        data = json.load(f)

    categories = {}
    undefined_errors = []

    # PHPStan JSON format: {'files': {'path/to/file': {'messages': [{'message': '...', 'line': 1}]}}}
    # Or strict error list? Usually the above.
    ...
    print("\n--- Critical Errors (Undefined/Not Found) ---")
    # Limit to first 50 to avoid flooding
    for err in undefined_errors[:100]:
        print(err)
        
    print(f"\nTotal Critical Errors: {len(undefined_errors)}")
